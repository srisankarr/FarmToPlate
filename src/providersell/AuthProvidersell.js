...
export const useAuthh = () => useContext(AuthContext);

export const AuthProviderr = ({ children }) => {
  const [user, setUser] = useState(null);
  const [message,setmessage]=useState('');
  const [isLoggedIn,setLoggedin]=useState(false);
  const [profi, setProfile] = useState(null);
  ...
  const login = async (credentials) => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/loginsell', credentials);
...
const clearMessage=()=>{
  ...
  const signup = async (userData) => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/signupsell', userData);
  ...
  const logout = async () => {
    try {
      ...
      await axios.post('http://localhost:5000/api/logoutsell');
  ...
  const fetchProfile = async() =>{
    ...
    try {
      const response = await axios.get('http://localhost:5000/api/profilesell', {
        headers: {
  ...
  const forgotPassword = async (email) => {
    try {
      const response = await axios.post('http://localhost:5000/api/forgot-passwordsell', { email });
      setmessage(response.data.message);
  ...
  const resetPassword = async (email, newPassword, confirmPassword) => {
    try {
      const response = await axios.post('http://localhost:5000/api/reset-password-sell', {
        email,
        newPassword,
        confirmPassword,
      });
  ...
  const addToCart = async (productId, quantity) => {
    try {
     ...
     const response= await axios.post(`http://localhost:5000/api/cart-sell/add/${productId}`, { quantity }, { headers: { Authorization:` Bearer ${token} `} });
  ...
  return (
    <AuthContext.Provider value={{ user, login, signup, logout,message,isLoggedIn,forgotPassword,resetPassword,redirectToResetPassword,resetRedirectToResetPassword,clearMessage,addToCart,profi}}>
      {children}
